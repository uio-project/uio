...
`uio/core/attribution.py` and prepended to every agent run that carries a
recognised `vcs-identity` role (`planner`, `coder`, or `reviewer`).
...
|---|---|
| Issue comments and PR/MR comments | A one-line footer: `🤖 Generated by **uio / <Display Name>** · <agent-name>` |
...
2. Requires every issue or PR comment body to end with:
   ```
   ---
   🤖 Generated by **uio / AI Coder** · my-coder-agent
   ```
